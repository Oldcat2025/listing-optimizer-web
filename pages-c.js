...
page('cfg-rules', {
  hidden:true,   /* [fix 09-19] 数据源「合规规则」当前 0 行；页面在、数据不在 = 看起来能用其实空 → 先隐藏，待规则入库后再放出来 */
  roles:['管理员'],
  guide:[
...
page('cfg-forbidden', {
  ...
  guide:[
    ...
    '词条<b>只增不删</b>（导入是追加）——历史判定要能复现，靠的就是词条留在库里。',
    '⚠️ 说明：当前版本<b>还没有「停用某个词条」的开关</b>，只能新增/导入。',
  ],
  spec:{
    q:'违禁词库当前是什么，各词的风险等级与替换建议。',
    acts:['新增词条','导入 CSV','按站点覆盖'],
    wf:['无（合规检查读取）'],
    reads:['forbidden_word'],
    writes:['forbidden_word','audit_log'],
    limits:['<b>部分匹配是陷阱</b>：bra 会命中 embrace，必须按完整单词匹配','词条只增不删（当前无停用/删除入口）——保留历史才能复现旧任务的判定']
  },
    body:function(){
...
page('cfg-prompt', {
  roles:['管理员'],
  guide:[
    '「AI 指令」就是告诉 AI 怎么写的那段话。',
    '⚠️ <b>当前它写在各工作流里（还没进数据库）</b>——数据库版本管理待接入。',
    '接入后的好处：能对比新旧、能拿历史任务回测、出问题能一键退回、还能回答「这条文案是哪版指令写的」。',
    '<b>规划：新版本要回测优于基线才能设为默认</b>；本页目前没有「新建/回测/设默认」入口。',
  ],
  spec:{
...
page('fb-publish', {
  ...
  spec:{
    ...
    var el = toolbar([kwBox('搜索 SKU'), kwBtn(), mktSel()], [], {tight:true}) + '<div id="fb-publish-root">' + ghost('正在加载已上架待登记…') + '</div>';
    setTimeout(function(){
      API.table('SKU_输入表', {'处理状态':'COMPLETED'}, 200).then(function(r){
        var root = document.getElementById('fb-publish-root');
        if (!root) return;
        if (!r.ok || !r.data || r.data.success === false) { root.innerHTML = callout('stop','数据加载失败',(r.data&&r.data.error)||'请检查网络或稍后重试'); return; }
        var rows = (r.data.data||[]).filter(function(x){ return x && x['SKU'] && mktHit(x) && kwHit(x); });
        if (!rows.length) { root.innerHTML = callout('warn','暂无数据','该功能还没有数据，接入数据源后显示实际内容。'); return; }
...
page('adm-audit', {
  ...
  spec:{
    ...
    acts:['筛选','导出','按对象追溯'],
    wf:['无'],
    reads:['运行日志表（run_log，追加式）'],
    writes:['无'],
    limits:['操作记录<b>只增不改不删</b>','改判、配置变更、密钥轮换、规则复核必须全部留痕']
  },
    body:function(){
    var el = toolbar([kwBox('搜索 SKU / 运行ID / 执行人'), kwBtn(), mktSel()], [], {tight:true}) + '<div id="adm-audit-root">' + ghost('正在加载操作记录…') + '</div>';
    setTimeout(function(){
      API.table('运行日志表', {}, 200).then(function(r){
        var root = document.getElementById('adm-audit-root');
        if (!root) return;
        if (!r.ok || !r.data || r.data.success === false) { root.innerHTML = callout('stop','数据加载失败',(r.data&&r.data.error)||'请检查网络或稍后重试'); return; }
        var rows = (r.data.data||[]).filter(function(x){ return x && x['运行ID'] && mktHit(x) && kwHit(x); });
        if (!rows.length) { root.innerHTML = callout('warn','暂无数据','该功能还没有数据，接入数据源后显示实际内容。'); return; }
